...
class BalanceViewTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        Payment.objects.create(
            loan_id=cls.loan, status='made', date=datetime(2019,4,24).astimezone(tz=timezone.utc), amount=Decimal('200')
        )
        Payment.objects.create(
            loan_id=cls.loan, status='made', date=datetime(2019,5,24).astimezone(tz=timezone.utc), amount=Decimal('200')
        )

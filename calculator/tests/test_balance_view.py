...
class BalanceViewTest(TestCase):
    ...
    def test_get_balance(self):
        ...
        response = self.client.get(
            reverse("get_balance", kwargs={"pk": self.loan.pk}), valid_payload
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {"balance": Decimal("600")})

    def test_get_balance_without_payload(self):
        response = self.client.get(reverse("get_balance", kwargs={"pk": self.loan.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_balance_invalid_loan(self):
        response = self.client.get(reverse("get_balance", kwargs={"pk": 850}))
